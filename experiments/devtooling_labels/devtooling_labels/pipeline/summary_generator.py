...
class SummaryGeneratorStep:
    ...
    def run(self, force_refresh: bool = False):
        ...
        # Check if summaries already exist and not forcing refresh
        # This check is a bit tricky if we want to support partial processing or adding new summaries.
        # For now, if the summarized file exists and not force_refresh, we assume it's complete.
        # A more robust check would compare repo_artifact_ids.
        ...
        # Filter out repositories that already have summaries if we were to implement incremental updates
        # For now, we process all or none based on force_refresh and file existence.
